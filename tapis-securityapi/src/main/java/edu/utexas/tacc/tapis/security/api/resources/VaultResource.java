     ...
     */ 
     ...
     @Operation(
             description = "Read a versioned secret. "
                           ...
                           + "    - keytype: *sshkey* | password | accesskey | token | cert\n"
                           + "  - **dbcred**\n"
     ...
     @Operation(
             description = "Create or update a secret. "
                           ...
                           + "### Generating Secrets\n"
                           + ""
                           + "Passwords and public/private key pairs appropriate for Tapis use can "
                           + "be generated as part of this secret write call.  To direct SK to "
                           + "create a secret, assign the special value `<generate-secret>` to "
                           + "a key.  When SK detects this value, it generates a password or key pair depending "
                           + "on context, and replaces the `<generate-secret>` text with the generated "
                           + "secret.  In the case of a key pair, both the public and private keys are "
                           + "saved.\n\n"
                           + ""
                           + "Key pairs are always generated for secrets of type JWTSigning, while "
                           + "passwords are generated for all other secret types unless the key "
                           + "is named *privateKey*.\n\n"
                           + ""
                           + "To generate a key pair, insert the following "
                           + "key/value pair into the payload's data map:\n\n"
                           + ""
                           + "    key=\"privateKey\", value=\"<generate-secret>\"\n\n"
                           + ""
                           + "When the key pair is generated, the above key/value item is replaced by "
                           + "these two key/value pairs:\n\n"
                           + ""
                           + "    key=\"privateKey\", value=<private key in pem format>\n"
                           + "    key=\"publicKey\",  value=<public key in pem format>\n\n"
                           + ""
                           + "In non-JWTSigning secret types, passwords are generated "
                           + "whenever the following key/value pair is encountered in the payload's "
                           + "data map:\n\n"
                           + ""
