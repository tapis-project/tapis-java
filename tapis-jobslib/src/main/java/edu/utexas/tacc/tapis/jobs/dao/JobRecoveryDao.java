  ...
  extends AbstractDao
{
  ...
  /* getJobRecovery:                                                       */
  /* ---------------------------------------------------------------------- */
  public List<JobRecovery> getJobRecovery() 
    throws TapisException
  {
      ...
      try
      {
          ...
          String sql = SqlStatements.SELECT_JOBRECOVERY;
          ...
          JobRecovery obj = populateJobRecovery(rs);
          while (obj != null) {
            list.add(obj);
            obj = populateJobRecovery(rs);
          }
      ...
      catch (Exception e)
      {
          ...
          String msg = MsgUtils.getMsg("DB_SELECT_UUID_ERROR", "JobRecovery", "allUUIDs", e.getMessage());
          _log.error(msg, e);
  ...
  /* populateJobRecovery:                                                  */
  /* ---------------------------------------------------------------------- */
  /** Populate a new JobRecovery object with a record retrieved from the 
   ...
   * JobRecovery object.  The result set is not closed by this method.
   * 
   ...
   */
  private JobRecovery populateJobRecovery(ResultSet rs)
   throws TapisJDBCException
  {
    ...
    // Populate the JobRecovery object using table definition field order,
    // which is the order specified in all calling methods.
    JobRecovery obj = new JobRecovery();
    try {
        obj.setId(rs.getInt(1));
        obj.setTenantId(rs.getString(2));
        obj.setConditionCode(rs.getString(3));
        obj.setTesterType(rs.getString(4));
        obj.setTesterParms(rs.getString(5));
        obj.setPolicyType(rs.getString(6));
        obj.setPolicyParms(rs.getString(7));
        obj.setNumAttempts(rs.getInt(8));
        obj.setNextAttempt(rs.getTimestamp(9).toInstant());
        obj.setCreated(rs.getTimestamp(10).toInstant());
        obj.setLastUpdated(rs.getTimestamp(11).toInstant());
        obj.setTesterHash(rs.getString(12));
