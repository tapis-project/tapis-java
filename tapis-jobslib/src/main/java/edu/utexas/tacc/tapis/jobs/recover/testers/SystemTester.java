...
extends AbsTester
{
   ...
   /* ********************************************************************** */
   // Tracing.
   private static final Logger _log = LoggerFactory.getLogger(SystemTester.class);
   ...
   /* constructor:                                                                 */
   /* ---------------------------------------------------------------------------- */
   public SystemTester(JobRecovery jobRecovery)
   {
       super(jobRecovery);
   }
    ...
    throws JobRecoveryAbortException
   {
       ...
       SystemsClient sysClient = null;
       try {
           sysClient = ServiceClients.getInstance().
                           getClient(_owner, _tenantId, SystemsClient.class);
       }
       catch (Exception e) {
           String msg = MsgUtils.getMsg("TAPIS_CLIENT_NOT_FOUND", "Systems", _tenantId, _owner);
    ...
    throws JobRecoveryAbortException
   {
       ...
       // We need the system owner to be specified.
       ...
       // We need the system name to be specified.
       _sysName = testerParameters.get("name");
       if (StringUtils.isBlank(_sysName)) {
           String msg = MsgUtils.getMsg("TAPIS_NULL_PARAMETER", "validateTesterParameters", 
                                        "name");
           _log.error(msg);
           throw new JobRecoveryAbortException(msg);
